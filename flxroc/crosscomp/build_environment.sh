...
prechecks() {
    ...
    if ! [ -f ../server.py -a -f ../client.py ]; then feedback 2 "Must run from within crosscomp directory in base flxroc folder."; fi
    feedback 1337 "Passed all pre-checks"
    for arg in "$@"; do
        if [ "$arg" == "--rebuild-containers" ]; then
            feedback 93 "REBUILD CONTAINERS ENABLED: Removing previous docker FLXROC containers in 5 seconds..."
            sleep 10
...
make_containers() {
    ...
    for xc_dockimg in $handled_architectures; do
        ...
        docker image ls | grep "$xc_dockimg" | grep "FLXROC" 2> /dev/null 1> /dev/null
        rcCheck=$?
        if [ $rcCheck -eq 0 -a $rebuild_containers == false ]; then
            ...
            if [ $rebuild_containers ]; then
                cid=$(docker image ls | grep "$xc_dockimg" | grep "FLXROC" | awk '{print $3}')
...
WORKDIR /flxroc
EOF
            docker build -t "$xc_dockimg:FLXROC" -f "./dockers/$xc_arch_str.Dockerfile" ./dockers 2> $archoutfile >> $archoutfile &
            ...
            docker image ls "$xc_dockimg:FLXROC" | grep FLXROC
            rcCheck=$?
...
docker run -it --rm -v $project:/flxroc $xc_dockimg:FLXROC /bin/bash -c "cd /flxroc; if [ -d crosscomp/pyvenv-$xc_arch_str ] ; then rm -Rf crosscomp/pyvenv-$xc_arch_str; fi; if [ -d modules/payloads ] ; then rm -Rf modules/payloads; fi; if [ -d modules/stagers ] ; then rm -Rf modules/stagers; fi; echo $nixreqs | base64 -d > crosscomp/requirements_b.txt; python3 -m venv crosscomp/pyvenv-$xc_arch_str; source crosscomp/pyvenv-$xc_arch_str/bin/activate; python -m pip install --upgrade pip 'setuptools<45.0.0'; pip install wheel; pip install -r crosscomp/requirements_b.txt; python client.py --freeze --name \${bin_target}.${xc_arch} \$si_host \$si_port; rm -f ./\${bin_target}*; deactivate;"
EOF
