...
prechecks() {
    ...
    for arg in "$@"; do
        if [ "$arg" == "--rebuild" ]; then 
            feedback 93 "REBUILD ENABLED: Removing previous docker FLXROC containers in 3 seconds..."
            sleep 5
            rebuild=true
        fi
    done
...
RUN mkdir /flxroc
RUN echo $nixreqs | base64 -d > /flxroc/build_requirements.txt
WORKDIR /flxroc
RUN pip install -r build_requirements.txt
EOF
            docker build -t "$xc_dockimg:FLXROC" -f "./build/$xc_arch_str.Dockerfile" ./build 2> $archoutfile >> $archoutfile &
...
docker run -it --rm -v $project:/flxroc $xc_dockimg:FLXROC /bin/bash -c "cd /flxroc; python client.py --freeze --name \${bin_target}.${xc_arch} \$si_host \$si_port; rm -f ./\${bin_target}*;"
EOF
